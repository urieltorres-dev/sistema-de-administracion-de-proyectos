...
<div class="flex flex-col">
    ...
    <div class="rounded overflow-hidden shadow bg-white mx-2 w-full">
        <div class="px-6 py-2 border-b border-light-grey flex justify-between items-center">
            <div class="font-bold text-xl">Clientes</div>
            @if (auth()->user()->usertype == 'admin')
            <a href="{{route('clients.create')}}" class="bg-green-500 hover:bg-green-800 text-white font-bold py-2 px-4 rounded-full">
                Agregar cliente
            </a>
            @endif
        </div>
    ...
    <div class="container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6 px-2 mt-2">
        ...
        <div class="rounded rounded-t-lg overflow-hidden shadow max-w-xs">
            <div class="text-center px-3 pb-6 pt-2">
                <h3 class="text-black text-sm bold font-sans">{{$client->name . ' ' . $client->lastname}}</h3>
                <p class="mt-2 font-sans font-light text-grey-700">{{$client->company}}</p>
            </div>
            <div class="flex justify-center pb-3 text-grey-dark">
                @if (auth()->user()->usertype == 'admin')
                <form action="{{route('clients.destroy', $client)}}" method="POST" novalidate class="delete-form">
                    @csrf
                    @method('DELETE')
                ...
                <form action="{{route('clients.edit', $client)}}" method="GET" novalidate>
                    @csrf
